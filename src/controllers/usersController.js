...
const usersController = {
  createUser: async (req, res) => {
    ...
    if (!email || !password)
      return res.status(400).json({ success: false, response: "Los parámetros email y password son obligatorios" })
  ...
  deleteUser: (req, res) => {
    ...
    User.findByIdAndDelete(id)
      .then(user => res.status(200).json({ success: true, response: { userDeleted: user } }))
      .catch(error => res.status(400).json({ success: false, error }))
  },
}
